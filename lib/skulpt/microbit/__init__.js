...
var $builtinmodule = function (name) {
	...
	var mb = {
		...
		init: function() {
			...
			html += '<div id="mb_tabs">';
			html += '<ul>';
			html += '<li><a href="#mb_tabs_tools">Micro:bit 工具</a></li>';
			html += '<li><a href="#mb_tabs_help">帮助与提示</a></li>';
			html += '<li><a href="#mb_tabs_accelerometer">加速度计</a></li>';
			html += '<li><a href="#mb_tabs_compass">指南针</a></li>';
			html += '<li><a href="#mb_tabs_pins">I/O 引脚</a></li>';
			html += '<li><a href="#mb_tabs_thermometer">温度计</a></li>';
			html += '<li><a href="#mb_tabs_radio">无线电</a></li>';
			html += '</ul>';
			html += '<div id="mb_tabs_radio">';
			html += '<div><h3>设置:</h3>';
			html += '<label for="mb_radio_channel">频道 (0-100): </label>';
			html += '<input name="mb_radio_channel" id="mb_radio_channel" value="7">';
			html += '<label for="mb_radio_address">地址 (32 bit hex): </label>';
			html += '<input name="mb_radio_address" id="mb_radio_address" value="0x75626974">';
			html += '<label for="mb_radio_group">范围 (0-255): </label>';
			html += '<input name="mb_radio_group" id="mb_radio_group" value="0">';
			html += '<label for="mb_radio_rate">数据数率: </label>';
			html += '<select name="mb_radio_rate" id="mb_radio_rate"><option value="250">数率_250KBIT</option><option value="1000" selected="selected">数率_1MBIT</option><option value="2000">数率_2MBIT</option></select>';
			html += '<button id="mb_btn_radio_update">更新</button>';
			html += '<div id="mb_radio_status"></div>';
			html += '</div>';
			html += '<div><h3>数据:</h3>';
			html += '<label for="mb_radio_message">消息: </label>';
			html += '<input name="mb_radio_message" id="mb_radio_message">';
			html += '<button id="mb_btn_radio_send">发送</button>';
			html += '</div>';
			html += '</div>';
			html += '<div id="mb_tabs_thermometer">';
			html += '温度计: <span id="mb_therm_value">23</span>&deg;C<div class="mb_slider" id="mb_therm_slider"></div>';
			html += '</div>';
			html += '<div id="mb_tabs_compass">';
			html += '调整指南针方向: <div class="mb_slider_com" id="mb_com_slider_heading"></div><div id="mb_com_heading"><span id="mb_com_value_heading">0</span><div id="mb_compass_pointer"></div></div>';
			...
			html += '<label for="mb_pin_num">管脚</label> ';
			...
			html += '<h3>手势</h3>';
			html += '<span class="mb_help_link mb_gesture">上</span>';
			html += '<span class="mb_help_link mb_gesture">下</span>';
			html += '<span class="mb_help_link mb_gesture">左</span>';
			html += '<span class="mb_help_link mb_gesture">右</span>';
			html += '<span class="mb_help_link mb_gesture">朝上</span>';
			html += '<span class="mb_help_link mb_gesture">朝下</span>';
			html += '<span class="mb_help_link mb_gesture">自由落地</span>';
	...
	//		html += '<a class="mb_help_link" id="mb_btn_download_hex" href="#"><i class="fa fa-download"></i> Download HEX</a>';
			html += '<a class="mb_help_link" id="mb_btn_both">按下 A + B</a>';
			html += '</div>';
			html += '<div id="mb_tabs_help">';
			html += '<a class="mb_help_link" href="https://microbit-micropython.readthedocs.io/en/latest/tutorials/hello.html" target="_blank"><i class="fa fa-question-circle"></i> 帮助与代码示例</a>';
			html += '<a class="mb_help_link" href="https://www.microbit.co.uk/" target="_blank"><i class="fa fa-external-link"></i> 官方网站</a>';
			html += '<a class="mb_help_link" href="http://blog.withcode.uk/tag/microbit/" target="_blank"><i class="fa fa-check"></i> 开发者博客</a>';
			html += '<a class="mb_help_link" href="http://cheat.microbit-playground.co.uk/" target="_blank"><i class="fa fa-smile-o"></i> 备忘单</a>';
			html += '</div>';
			html += '</div>';
			...
			html += '</div>';
			html += '<div class="mb_info">这个micro:bit模拟器是一个非官方的测试版本，主要是在Python语言下进行仿真micro:bit。</div>';
			...
			$('#mb_btn_radio_send').button().click(function() {
				if(typeof(radio) !== "undefined") {
					if(radio.fn_receive) {
						...
						$('#mb_radio_status').html('未调整到与micro:bit无线电相同的设置。检查设置并按更新');
					}
				} else {
					$('#mb_radio_status').html('无线电模块未找到: 你是否 "导入无线电"?');
				}
			});
			...
			$('#mb_btn_radio_update').button().click(function() {
				...
				mod.radio.channel = getValueInRange('mb_radio_channel', 0, 100, '请输入0到100之前的频道号码');
				mod.radio.group = getValueInRange('mb_radio_group', 0, 255, '请输入0到255之间的数字');
				mod.radio.address = getHexValue('mb_radio_address', '请输入有效的32位十六进制数字');
				mod.radio.data_rate = $('#mb_radio_rate').val();
				if(typeof(radio) !== "undefined") {
					var tuned = true;

					if(mod.radio.channel != radio.channel) {
						$('#mb_radio_status').html("频道不匹配：目前设置为 " + radio.channel);
						tuned = false;
					}
					if(mod.radio.group != radio.group) {
						$('#mb_radio_status').html("范围设置不匹配：当前设置为 " + radio.group);
						tuned = false;
					}
					if(mod.radio.address != radio.address) {
						$('#mb_radio_status').html("地址不匹配：当前设置为 " + radio.address.toString(16));
						tuned = false;
					}
					if(mod.radio.data_rate != radio.data_rate){
						$('#mb_radio_status').html("数据速率不匹配：当前设置为 " + radio.data_rate);
						tuned = false;
					}
					if(tuned) {
						$('#mb_radio_status').html("收听无线电模块");
						radio.fn_send = function(message) { // send from microbit to UI
							$('#mb_radio_status').html("接收到消息: " + message);
						}
						radio.fn_receive = function(message) { // send from UI to microbit
							if(radio.buffer.length < radio.queue) {
								radio.buffer.push(message);
							} else {
								$('#mb_radio_status').html("队列中发现有微型无线电");
							}
					...
					$('#mb_radio_status').html('无线电模块未检测到你是否"导入无线电"?');
				}
			});
			...
			function updatePinSpinner(pinNumber) {
				...
				switch(pinNumber) {
					...
					case 2:
						...
						html = '<button id="mb_btn_pin_touch">触摸</button>';
					...
					case 10:
						pinType = "模拟";
						pin = mod['pin' + pinNumber];
						html = '值: <span id="mb_pin_analog_value">' + pin.value + '</span><div class="mb_slider" id="mb_pin_analog"></div> PWM 周期: ' + pin.period_us / 1000 + 'ms';
						$('#mb_pin_detail').html(html);
					...
					case 20:
						...
						html = '目前切换: <button id="mb_btn_pin_digital">' + (pin.value? '开' : '关') + '</button>';

						$('#mb_pin_detail').html(html);
						$('#mb_btn_pin_digital').button().click(function() {
							pin.value = !pin.value;
							$('#mb_btn_pin_digital').button('option', 'label', pin.value? '开' : '关');
						});
						break;
					default:
						pinType = "+3v";
