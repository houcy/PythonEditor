...
var PythonIDE = {
	...
	welcomeMessage: "按 Ctrl+Enter 键执行",
	...
	recover: function() {
		...
		var html = '<p>每次你执行它, 你的程序代码会储存一个备份在你的浏览器.</p>';
		html += '<p>目前存储的程序代码备份:' + PythonIDE.vault.length + '</p>';
		html += '<div id="slider_recover"></div><span id="recover_time">请拖拉滑杆往回的时间来恢复你的程序代码</span>';

		html += '<p><button id="btn_recover">恢复</button></p>';
		html += '<p>如果有多人使用同一个名称登入计算机, 你可能希望刪除所有存储的恢复备份.</p><p><i class="fa fa-warning"></i> 请注意: 一旦你按下刪除全部按鈕就会马上刪除, 无法再救回.</p>';
		html += '<p><button id="btn_recover_clear">刪除全部</button></p>';
	...
	runCode: function(runMode) {
		...
		html += '<pre id="consoleOut"><div id="watch"><h2>变量:</h2></div></pre>';
		html += '</pre>';
		if(code.indexOf("turtle") > 0) {
			html += '<div id="canvas"></div>';
		}
		html += '<div><button id="btn_stop">停止</button><button id="btn_hideConsole">隐藏</button></div>';
		...
		if(runMode != "normal") {
			handlers["Sk.debug"] = function(susp) {
				...
				var html = '<h2>全域变量:</h2><table><tr><th>名称</th><th>型态</th><th>Value</th></tr>';
				...
				for(var key in susp.child.$gbl) {
					...
					if(val && val.length && val.length > 20) {
						...
						val = '<span class="debug_expand_zone" id="debug_expand_' + eH.id + '">' + val.substring(0, 17) + '<img src="/media/tools.png" class="debug_expand" title="点击显示完整值"></span>';
					}
			...
			if(code.indexOf("while ") > -1) {
				console.log("Crash prevention mode enabled: This happens when your code includes an infinite loop. Your code will run much more slowly in this mode.");
				...
				handlers["Sk.debug"] = function(susp) {
					...
					if(new Date().getTime() - startTime > 100) {
						...
						var p = new Promise(function(resolve, reject) {
							setTimeout(function() {
								PythonIDE.showHint("限制执行速度避免浏览器产生错误: " + (lineCount * 10) +  " 行/秒");
								lineCount = 0;
								return resolve(susp.resume());
							}, 50);
	...
	handleError:function (err){
		...
		var html = '<span class="error">' + PythonIDE.sanitize(err.toString()) + '</span>';
	...
	showShare: function(){
		...
		var embed = ('https://www.gdplume.com' + window.location.pathname).replace('python/', 'embed/');
	...
	save: function() {
		PythonIDE.showHint("存储中...");
		var code = PythonIDE.files['mycode.py'];

		if(PythonIDE.countFiles() > 1) {
			code = JSON.stringify(PythonIDE.files);
		}
		//console.log(encodeURIComponent(btoa(code)))
		$.getJSON('lib/api.php', {
			cmd: 'save',
			code: encodeURIComponent(btoa(code))
		}, function(data) {
			//console.log(data);
			var link = 'https://www.gdplume.com/python'+data.hash;
	...
	init: function(style) {
			...
			switch(e.currentTarget.id) {
				case 'btn_file_rename':
					...
					if(!newFileName.match(/^[A-Za-z0-9_.]+$/)){
						PythonIDE.showHint("不合法的文件名称");
						break;
					}
					if(PythonIDE.files[newFileName]) {
						PythonIDE.showHint('文件名称已经存在');
						break;
					}
		...
		if(style == "run") {
			...
			PythonIDE.whenFinished = function() {
				...
				var html = '<div><a class="nounderline" href="http://www.gdplume.com" target="_blank"><h1 id="title" onmouseover="animateTitle(\'create.gdplume.com\', \'title_text\')"><span class="brackets">{</span><span id="title_text">gdplume.com</span><span class="brackets">}</span></h1></a></div>';
				PythonIDE.python.output(html + '<p>This python app was written using <a href="https://www.gdplume.com">create.withcode.uk</a>. <a href="' + link + '">Click here to edit the python code and create/share your own version</a> or check out <a href="http://blog.gdplume.com">blog.gdplume.com</a> for ideas, tips and resources</p> <button id="btn_run_again">Run again</button>');
