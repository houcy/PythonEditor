...
Python Editor简体中文版 翻译者: LEON 2018/3
*/
...
function web_editor(config) {
    ...
    function setupEditor(message) {
        ...
        $("#loadXmlButton").click(function (e) {
            // Init load event.
            var loadInput = document.getElementById('load');
            loadInput.addEventListener('change', loadxmlfile, false);
            loadInput.click();
        });
        ...
        function loadxmlfile(event) {
             ...
             reader.onloadend = function(event) {
                  ...
                  if (target.readyState == 2) {
                      ...
                      if (count && confirm('确认取代存在的积木?\n按"取消"钮是合并积木.')) {
                          Blockly.mainWorkspace.clear();
                      }
                      Blockly.Xml.domToWorkspace(Blockly.mainWorkspace, xml);
                      setName(files[0].name.replace('.xml', ''));
                 }
        ...
        $("#saveXmlButton").click(function (e) {
            var xml = Blockly.Xml.workspaceToDom(Blockly.mainWorkspace);
            
            filename = $("#script-name").text();
        ...
        $("#command-runscript").click(function (e) {
            // alert("run simulator");
            var output = EDITOR.getCode();
            if (typeof(Storage) !== "undefined") {
                  sessionStorage.setItem("pythoncode", output); 
            } else {
                 alert("浏览器不支技Web Storage!")
            }             
       });
    ...
    function doBlockly() {
        ...
        if(blockly.is(':visible')) {
            ...
            $('#editor').attr('title', '当使用积木程序时, 程序代码编辑器是只读, 不允许编辑程序代码.');
            $("#command-snippet").off('click');
            $("#command-snippet").click(function () {
